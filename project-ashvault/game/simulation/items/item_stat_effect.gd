...
func validation_error() -> String:
	if not effect_id.begins_with("item_effect.") or not StableIdContract.is_valid(effect_id):
		return "Item stat effect requires a stable item_effect ID."
...
static func list_error(effects: Array) -> String:
	...
	for effect: Variant in effects:
		if not effect is ItemStatEffect:
			return "Item contributions require ItemStatEffect Resources."
		var error: String = effect.validation_error()
		if not error.is_empty():
			return error
		if seen.has(effect.effect_id):
			return "Item stat effect IDs must be unique within their source."
